...
class AbstractServerManager():
    ...
    def update_transfer_fench_users(self, curr_transfers):
     ...
     2. 要求：最新的提供服务的用户 配置信息（包括端口、密码、加密方式等等）
        返回值 user_cfgs 格式要求如：
        {23123:{'method':'rc4-md5','passwd':'I7S3dh2'},23127:{'method':'chacha20','passwd':'2Di3h9'}}
     :return :
     '''
     pass

    def load_default_cfgs(self):
        return self.spool.config

    def load_user_cfgs(self, user_cfg):
        return self.load_default_cfgs().update(user_cfg)
    ...
    def loop_server(self):
        ...
        for port in in_service_port_set:
            ...
            if user_cfg != self.last_user_cfgs[port]:
                logging.info('manager reload server at port [%s] because config changed: %s' % (port, user_cfg))
                self.spool.cb_del_server(port)
        ...
        for port in not_in_port_set:
            if port > 0 and port < 65536 :
                cfg = self.load_user_cfgs(user_cfgs[port])
                logging.info('manager start server at port [%s],config: %s' % (port, cfg))
                self.spool.new_server(port, cfg)
                # @@@@端口新开启后，流量戳置零
                self.last_transfer_stamps[port] = (0, 0)
            else:
